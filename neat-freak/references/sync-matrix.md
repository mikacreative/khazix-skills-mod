...
| 项目阶段完成 / 里程碑变化 | Obsidian `20 Projects/<项目>/` 当前状态 · 本次进展 · 下一步任务 · 工作日志 |
| 新决策 / 推翻旧决策 | 项目根 markdown(若影响 agent 执行) · `docs/architecture.md`(若影响系统设计) · Obsidian `20 Projects/<项目>/` 关键决策 |
| 风险 / 阻塞变化 | `docs/operator-runbook.md`(若影响运维) · Obsidian `20 Projects/<项目>/` 风险 / 阻塞 |
...
Obsidian 也要按跨项目影响检查：如果 `20 Projects` 下存在上游和下游两个项目文件夹/笔记，不能只更新当前代码仓库对应的那一个。上游 API、共享配置、交付状态、阻塞原因发生变化时，下游项目笔记里的当前状态、下一步任务和风险也要对齐。
...
5. **Obsidian 项目笔记**:项目状态、关键决策、本次进展、下一步任务、风险、工作日志
